...
from nbdime.args import args_for_browse, args_for_server, prettyprint_config_from_args
...
from nbdime.webapp.nbdiffweb import build_arg_parser
...
def compare_notebook_dirs(
    ...
    recursive: Annotated[bool, typer.Option("--recursive", "-r", help="search folders recursively?")] = False,
    ...
    if len(mismatched) > 0:
        ...
        for name, data in mismatched.items():
            answer = Prompt.ask(
                f"There were differences for [orange1]{name}[/orange1]. Would you like to examine them using on the command line or in a web browser?",
                choices=["c", "w", "n"],
                default="n",
            ...
            match answer:
                ...
                case "w":
                    source_path_url = str(data["notebook1_path"])
                    remote_path_url = str(data["notebook2_path"])
                    opts = build_arg_parser().parse_args([source_path_url, remote_path_url])
                    main_server(
                        on_port=lambda port, source_path_url=source_path_url, remote_path_url=remote_path_url, opts=opts: (
                            browse(
                                port=port,
                                rel_url="diff",
                                base=source_path_url,
                                remote=remote_path_url,
                ...
                case "n":
                    console.print("[yellow]Moving on...[/yellow]")
    ...
    if report:
        table = Table(show_lines=True, row_styles=["", Style(bgcolor="#222222")], box=box.HORIZONTALS)
        ...
        table.add_column("[bold yellow3]Status[/]", justify="center", style="cyan", no_wrap=True)
        table.add_column("[bold dodger_blue2]Source[/]", justify="center", style="cyan", no_wrap=True)
        ...
        for notebook in unique_notebook_names:
            ...
            if notebook in mismatched:
                diff_status = "[yellow]differ[/yellow1]"
            ...
            if notebook in deleted_notebooks1:
                source_status = "[orchid1]deleted[/orchid1]"
            elif notebook in deleted_notebooks2:
                remote_status = "[orchid1]deleted[/orchid1]"
        ...
        remove_which = Prompt.ask(
            prompt=(
                f"For those files which were identical, would you like to remove the \n[dodger_blue2]source:[/] [grey50]({source_path})[/], \n[green]remote:[/] [grey50]({remote_path})[/], \nor [dark_goldenrod]keep[/] both?"
            ),
